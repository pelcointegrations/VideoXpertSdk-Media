...
static const struct
{
  ...
  GST_VIDEO_FORMAT_I420, (D3DFORMAT)D3DFMT_YV12}, {
  GST_VIDEO_FORMAT_YV12, (D3DFORMAT)D3DFMT_YV12}, {
  GST_VIDEO_FORMAT_NV12, (D3DFORMAT)D3DFMT_NV12}, {
...
gst_d3d_surface_memory_share (GstMemory * mem, gssize offset, gssize size)
{
  ...
  gst_memory_init (GST_MEMORY_CAST (sub), (GstMemoryFlags)(GST_MINI_OBJECT_FLAGS (parent) | GST_MINI_OBJECT_FLAG_LOCK_READONLY), mem->allocator,
...
gst_d3dsurface_buffer_pool_new (GstD3DVideoSink * sink)
{
  ...
  pool = (GstD3DSurfaceBufferPool*)g_object_new (GST_TYPE_D3DSURFACE_BUFFER_POOL, NULL);
  gst_object_ref_sink (pool);
  pool->sink = (GstD3DVideoSink*)gst_object_ref (sink);
    ...
    GstStructure * config)
{
  ...
  if (pool->add_metavideo) {
    pool->allocator =
        (GstAllocator*)g_object_new (GST_TYPE_D3D_SURFACE_MEMORY_ALLOCATOR, NULL);
    ...
    GstBuffer ** buffer, GstBufferPoolAcquireParams * params)
{
  ...
  *buffer = gst_buffer_new ();
  ...
  gst_memory_init (GST_MEMORY_CAST (mem), (GstMemoryFlags)0, pool->allocator, NULL, size, 0, 0,
      size);

  mem->surface = surface;
  mem->sink = (GstD3DVideoSink*)gst_object_ref (sink);
  mem->x = mem->y = 0;
...
d3d_copy_buffer (GstD3DVideoSink * sink, GstBuffer * from, GstBuffer * to)
{
  ...
  switch (sink->format) {
    ...
    case GST_VIDEO_FORMAT_UYVY:{
      ...
      src = (guint8*)GST_VIDEO_FRAME_PLANE_DATA (&from_frame, 0);
      dst = (guint8*)GST_VIDEO_FRAME_PLANE_DATA (&to_frame, 0);
    ...
    case GST_VIDEO_FORMAT_NV12:{
      ...
      for (i = 0; i < 2; i++) {
        src = (guint8*)GST_VIDEO_FRAME_PLANE_DATA (&from_frame, i);
        dst = (guint8*)GST_VIDEO_FRAME_PLANE_DATA (&to_frame, i);
...
d3d_render_buffer (GstD3DVideoSink * sink, GstBuffer * buf)
{
  ...
  if (gst_buffer_n_memory (buf) != 1 ||
      ...
      !gst_memory_is_type (mem, GST_D3D_SURFACE_MEMORY_NAME)) {
    GstBuffer *tmp;
    GstBufferPoolAcquireParams params = { (GstFormat)0, };
...
d3d_class_init (GstD3DVideoSink * sink)
{
  ...
  WM_D3DVIDEO_NOTIFY_DEVICE_LOST =
      RegisterWindowMessageA("WM_D3DVIDEO_NOTIFY_DEVICE_LOST");
