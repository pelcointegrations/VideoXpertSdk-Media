...
gst_d3dvideosink_propose_allocation (GstBaseSink * bsink, GstQuery * query)
{
  ...
  GST_OBJECT_LOCK (sink);
  pool = sink->pool ?  (GstBufferPool*)gst_object_ref (sink->pool) : NULL;
  GST_OBJECT_UNLOCK (sink);
    ...
    GstStructure* structure)
{
    ...
    if ((e = gst_event_new_navigation(structure))) {
        ...
        gboolean plugin_init_d3d(GstPlugin* plugin)
    {
#ifdef ENABLE_NLS
        ...
        if (!gst_element_register(plugin, "d3dvideosinkpelco", 512,
            GST_TYPE_D3DVIDEOSINK))
            return FALSE;
        return TRUE;
    }

#define PACKAGE "PelcoD3D"
